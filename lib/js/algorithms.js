 ...
 * then do same operation for B in case B has other properties that A doesn't have
 */
function haveSameData(A, B){
    ...
    if(aKeys.length == bKeys.length){
        if (aKeys.every( (key) => B.hasOwnProperty(key) && A[key] == B[key] ) && bKeys.every( (key) => A.hasOwnProperty(key) && A[key] == B[key] )){
            return true;
        }
    }
    return false;
...
export default { getLowestAvailableInt, haveSameData }